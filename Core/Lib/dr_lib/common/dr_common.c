...
dr_file_iterator* dr_file_iterator_begin(const char* pFolderPath, dr_file_iterator* pState)
{
    ...
    //    return NULL;    /* Couldn't find anything. */
    //}
    if (f_opendir(dir, pFolderPath) != FR_OK) {
    ...
    for (;;) {
        ...
        dr_strcpy_s(pState->relativePath, sizeof(pState->relativePath), file_info.fname);
...
dr_file_iterator* dr_file_iterator_next(dr_file_iterator* pState)
{
    ...
    for (;;) {
        ...
        */
        if (f_readdir(pState->dir, &file_info) != FR_OK) {
            //log_error("Failed to read directory %s", dir_path);
        	f_closedir(pState->dir);
        	pState->dir = NULL;
            return NULL;
        }
...
void* dr_open_and_read_file_with_extra_data(const char* pFilePath, size_t* pFileSizeOut, size_t extraBytes)
{
    ...
    if (dr_fopen(&pFile, pFilePath, FA_OPEN_EXISTING | FA_READ) != 0) {
        return NULL;
    }
    ...
    pFileData = SDRAM_BASE_ADDR + 1048576; //1M
    if (pFileData == NULL) {
    	f_close(&pFile);
    ...
    f_read(&pFile, pFileData, (size_t)fileSize, &bytesRead);
